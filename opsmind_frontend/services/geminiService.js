...
// Gemini API Configuration - Read dynamically from window
const getGeminiConfig = () => ({
    apiKey: window.GEMINI_API_KEY || '',
    apiUrl: window.GEMINI_API_URL || 'https://generativelanguage.googleapis.com/v1/models/gemini-2.5-flash:generateContent'
});
...
const GeminiService = {
    ...
    async generateResponse(message, conversationHistory = []) {
        try {
            ...
            console.log('[Gemini] API Key configured:', config.apiKey.substring(0, 10) + '...');
            ...
            // Make the API request to Gemini using dynamic config
