...
3. Build and start Workflow Service
...
**Database seeding needed (db/seed.sql):**
```bash
# Run seed from inside container
docker-compose exec workflow-service sh
cd db
mysql -h opsmind-mysql -u root -proot workflow_db < seed.sql
```
